...
+(NSString *)timeCompareTimestamp:(NSString *)formatTime{
    
    NSString *timeStampString  = formatTime;
    ...
    int day_max = (currenttime - interval) /(24*60*60);
    
    if (day_max > 0) {
        
        return [NSString stringWithFormat:@"%d天前 %@",day_max,[dateString substringFromIndex:11]];
        
    }else{
        
        if ([[dateString substringToIndex:10] isEqualToString:[currentdate substringToIndex:10]]) {
            
            
            return [NSString stringWithFormat:@"%@",[dateString substringFromIndex:11]];
        }else{
            
            return [NSString stringWithFormat:@"昨天 %@",[dateString substringFromIndex:11]];
        }
    }
